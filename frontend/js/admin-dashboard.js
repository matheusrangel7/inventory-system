    ...
    tbody.innerHTML = users.map(u => `
                <tr class="align-top transition hover:bg-blue-50/40">
                    ...
                    <td class="${TABLE_ACTION_CELL_CLASS}">
                        ${renderTableActions([
                            { label: "Editar", variant: "secondary", title: "Editar utilizador", attrs: { "data-user-action": "edit", "data-user-id": getUserId(u) } },
                            { label: "Remover", variant: "danger", title: "Remover utilizador", attrs: { "data-user-action": "remove", "data-user-id": getUserId(u) } }
                        ])}
                    </td>
                </tr>
            `).join("");
...
function renderLocationsTable() {
    ...
    if (!locations.length) {
        renderEmptyRow(tbody, 3, "Nenhum local encontrado.");
        return;
    }
...
function renderUserLocationsCell(user) {
    ...
    if (!locationNames.length) {
        return `<span class="text-xs font-semibold text-gray-400">Sem salas</span>`;
    }
