...
const USER_SECURITY_ROLLBACK_REASON = "Ações de utilizadores, MFA, palavras-passe ou estado de registo não podem ser revertidas por segurança. Só a remoção/desativação de conta pode ser revertida.";
const ADMIN_ROLLBACK_SCOPE_REASON = "Rollback disponível apenas para Locais, Categorias, Features, Specs e remoção/desativação de contas.";
...
function getLogActionKey(log) {
    return String(getLogAction(log) || "").trim().toUpperCase();
}

function isRollbackFlagEnabled(log) {
...
function isUserAccountDeactivationLog(log) {
    ...
    if (getLogActionKey(log) !== "DELETE") return false;
...
function getLogRollbackReason(log) {
    if (isUserSecurityLog(log)) {
        return USER_SECURITY_ROLLBACK_REASON;
    }
    if (!isAdminRollbackableLogTable(log)) {
        return ADMIN_ROLLBACK_SCOPE_REASON;
    }
