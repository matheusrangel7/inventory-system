...
Novas abstrações devem resolver uma necessidade concreta. Não é necessário
adicionar repositories, DTOs ou uma camada global de use cases apenas para
seguir uma arquitetura teórica.

A reorganização do serviço de inventário permanece adiada até à integração das
alterações que estão a ser desenvolvidas nesse módulo.
